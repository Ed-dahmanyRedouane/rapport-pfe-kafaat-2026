...
% DOCUMENT TITLE
% ==========================================
% Rapport_PFE_ED-DAHMANY_v1_Draft
% Rapport_PFE_ED-DAHMANY_v2_Correction
% Rapport_PFE_ED-DAHMANY_Redouane_FINAL
% Rapport_PFE_ED-DAHMANY_Redouane_FINAL_V2
% ------------------------------------------
% Rapport_PFE_ED-DAHMANY_Redouane_NTTDATA_2026
% Rapport_PFE_KAFAAT_EMS_ED-DAHMANY_2026
% Memoire_Master_M2I_ED-DAHMANY_Redouane_2026
% Memoire_Master_M2I_NTTDATA_ED-DAHMANY_Redouane_2026
...
\addtolength{\cfttabnumwidth}{48pt} 
% ---------------------------------------------------------
...
\definecolor{azureBlue}{RGB}{0, 122, 204}  

% ==========================================
% CADRE DOUBLE - PAGE DE GARDE UNIQUEMENT
% ==========================================
\newcommand{\titlepageframe}{%
  \AddToShipoutPictureBG{%
    \begin{tikzpicture}[overlay]
      % Cadre extérieur (trait épais)
      ...
      % Cadre intérieur (trait fin, coins arrondis)
      \draw[royalBlue, line width=0.6pt, rounded corners=10pt]
...
\newcommand{\mycustomheader}{
    ...
    \begin{minipage}[b]{0.45\textwidth} 
        ...
        \raisebox{-0.4cm}{\includegraphics[height=2.2cm]{images/fst-logo.png}}
    \end{minipage}\par
    \vspace{-0.4cm} 
    \noindent
    \begin{tikzpicture}
...
\begin{tabular}{|l|l|l|l|}
  ...
  \hline
  \textbf{\color{primaryBlue}Pr.} &
  \textbf{\color{primaryBlue}Zaelouk Ouissam} &
  {\color{primaryBlue}\small Faculté des Sciences Tétouan} &
  {\color{primaryBlue}\bfseries Examinateur} \\
  \hline
  \rowcolor{lightBlue}
  \textbf{\color{primaryBlue}Pr.} &
  \textbf{\color{primaryBlue}Benameur Lamyae} &
  {\color{primaryBlue}\small Faculté des Sciences Tétouan} &
...
% ------------------------------------------
\begin{center}
  {\fontsize{14}{16}\selectfont\color{royalBlue}\bfseries Année Universitaire : 2025 - 2026\par}
\end{center}
...
Ensuite, je tiens à remercier chaleureusement mon encadrante universitaire, le Professeur \textbf{Benameur Lamyae}, pour son soutien constant, ses précieux conseils et son expertise tout au long de ce projet. Sa disponibilité, sa patience et sa bienveillance ont été d'une aide inestimable.
...
Je souhaite également remercier les membres du jury, le Professeur \textbf{Amjad Souad} et le Professeur \textbf{Zaelouk Ouissam}, d'avoir accepté d'évaluer ce travail.
...
\noindent
\textcolor{royalBlue}{\rule{0.35\textwidth}{1pt}} 
\hfill
\mbox{\fontsize{24}{28}\selectfont\color{royalBlue}\bfseries Liste des abréviations} 
...
% --- CHAPITRE 1 : START ---
\chapterpage{1}{Contexte général du projet} 
...
\vspace{-0.3cm}
NTT DATA, en tant que fournisseur mondial de services informatiques de confiance, propose une gamme complète de services pour aider les entreprises à relever les défis numériques et à innover. La figure suivante montre les services offerts par société NTT DATA \cite{ntt_global}
...
NTT DATA Maroc contribue également à l'écosystème technologique du pays en collaborant avec des partenaires locaux, des universités et des institutions pour promouvoir l'innovation, le développement des compétences et la croissance du secteur des technologies de l'information au Maroc. \cite{ntt_maroc}
...
\vspace{-0.4cm}
Comme la méthodologie de travail Scrum que nous utilisons est une approche Agile, l'outil \textbf{JIRA} s'est imposé comme la solution idéale pour gérer et suivre l'avancement de notre projet. \cite{jira_software}
...
\vspace{-0.4cm}
En résumé, l'adoption d'UML pour la conception de ce projet nous permet de maîtriser la complexité du module de facturation, d'encapsuler la logique métier et de fournir une documentation claire, tout en bénéficiant d'une transition fluide vers l'implémentation logicielle. \cite{uml_ibm}
...
Le diagramme de séquence montre l'ordre dans lequel les messages sont envoyés, ainsi que les objets qui envoient et reçoivent ces messages via des "lignes de vie" verticales. Ce diagramme permet de modéliser le comportement dynamique d'un système en montrant comment les composants (Frontend, Backend, Base de données) interagissent pour accomplir un processus métier spécifique. \cite{uml_ibm}

\vspace{-0.5cm}
\subsubsection{Diagramme de séquence « S'authentifier »}

\vspace{-0.4cm}
L'authentification est une étape cruciale pour sécuriser l'accès à la plateforme KAFA'AT EMS. Notre processus s'appuie sur les standards modernes de sécurité de l'architecture distribuée, notamment \textbf{Spring Security} et les jetons \textbf{JWT (JSON Web Token)} \cite{jwt_auth0}. 
...
\vspace{-0.4cm}
Les diagrammes d'état-transition (ou diagrammes d'états) sont des diagrammes utilisés dans le langage de modélisation unifié (UML) pour représenter le cycle de vie complet d'un objet ou d'un composant au sein d'un système. Ils fournissent une vue visuelle des différents états qu'une entité peut prendre au cours de son existence, en mettant l'accent sur les événements qui déclenchent le passage d'un état à un autre. Ces diagrammes sont constitués d'états (simples ou composites), de transitions, d'événements et de conditions de garde. Un état représente une condition ou une situation dans la vie d'un objet pendant laquelle il satisfait une certaine règle métier ou exécute une action continue, tandis que les transitions indiquent le changement d'état provoqué par un déclencheur spécifique.

Ces diagrammes sont particulièrement utiles pour modéliser des objets ayant un comportement dynamique complexe, des processus métier réactifs ou des flux de validation stricts dans un système. Ils permettent de visualiser l'évolution temporelle des entités et de comprendre les actions internes (telles que les opérations exécutées à l'entrée, en cours, ou à la sortie d'un état) qui interviennent tout au long du processus. \cite{uml_ibm}

Dans le contexte de notre plateforme \textbf{KAFA'AT EMS}, ces diagrammes nous permettront de modéliser avec précision le cycle de vie de nos entités métier critiques, notamment l'évolution et la validation des pointages (\textit{Worktime}), la logique du moteur de génération massive, ainsi que les phases de suivi et de recouvrement des factures.

\vspace{-0.4cm}
\subsubsection{Diagramme d'état-transition « Cycle de vie des pointages »}

\vspace{-0.4cm}
Ce premier diagramme d'état-transition modélise avec précision le cycle de vie de l'objet métier \texttt{Pointage} au sein de notre système. Afin d'illustrer la dynamique du processus de suivi du temps de travail, nous avons structuré ce diagramme en \textbf{états et transitions}, mettant en évidence les évolutions déclenchées par les acteurs ou le système.

Le cycle de vie se divise en deux phases critiques :
\begin{enumerate}[label=\textcolor{primaryBlue}{\arabic*.}, leftmargin=*]
    \item \textcolor{primaryBlue}{\textbf{Initialisation et Soumission :}} Le système agit comme un premier filtre conditionnel (Nœud de décision). Il vérifie de manière autonome si le cumul des heures déclarées ne dépasse pas le plafond journalier autorisé. Si cette condition de garde est respectée, le pointage transite vers l'état \textit{SOUMIS}.
    \item \textcolor{primaryBlue}{\textbf{Évaluation Administrative (État Composite) :}} Lors de la révision, le pointage entre dans un workflow décisionnel offrant trois transitions : l'approbation, le refus (avec motif) ou l'ajustement (\textit{Override}). Chaque transition déclenche des actions internes spécifiques (\textit{entry/do/exit}), incluant la mise à jour des états finaux et l'envoi d'alertes \textbf{SMTP} asynchrones.
\end{enumerate}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{images/etat-transition-pointage.png}
    \vspace{-0.9cm}
    \caption{Diagramme d'état-transition : Cycle de vie et validation d'un pointage}
    \label{fig:etat_transition_pointage}
\end{figure}

\vspace{-0.6cm}

\subsubsection{Diagramme d'état-transition « Génération Massive des Factures »}

\vspace{-0.4cm}
Ce deuxième diagramme d'état-transition illustre le cœur logique de notre module d'Invoicing. Le cycle de vie de la génération massive des factures est une opération critique qui exige un traitement par lots (\textit{Batch processing}) optimisé et une gestion rigoureuse des exceptions.

Le processus transite d'abord par un état d'initialisation incluant une vérification stricte des habilitations de l'administrateur via son Token JWT \cite{jwt_auth0} et l'extraction des pointages marqués comme \textit{APPROUVÉS}. L'architecture métier repose ensuite sur un \textbf{état composite} de traitement qui évalue chaque employé individuellement. Afin de prévenir les blocages de la chaîne, un nœud de décision intègre une gestion d'erreur interne : si un employé ne possède pas de paramètres financiers configurés (Groupe de Facturation/TJM manquant), le système dévie vers un état d'anomalie partielle avant de poursuivre l'itération sans interruption.

Pour les profils valides, le système passe à l'état de génération unitaire où il calcule les montants (HT et TTC), génère le document PDF via \textbf{JasperReports} \cite{jasperreports}, et prépare l'entité. Pour des raisons de performance, l'enregistrement en base de données s'effectue globalement via un état de persistance en masse (\textit{Batch Insert}). 

Enfin, le cycle de vie s'achève par des \textbf{états orthogonaux} (exécution parallèle) : l'envoi des courriels contenant les pièces jointes est délégué à un état de notification asynchrone, tandis que l'interface administrateur transite instantanément vers un état de synthèse détaillant le nombre de factures générées, le chiffre d'affaires total, et les éventuelles anomalies rencontrées.

\vspace{0.0cm}

\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{images/etat-transition-facturation.png}
    \vspace{-0.9cm}
    \caption{Diagramme d'état-transition : Cycle de vie de la génération massive des factures}
    \label{fig:etat_transition_facturation}
\end{figure}

\vspace{-0.4cm}

\subsubsection{Diagramme d'état-transition « Suivi et recouvrement des factures »}

\vspace{-0.4cm}
Ce dernier diagramme d'état-transition modélise le cycle de vie post-génération d'une facture, englobant les opérations de suivi, d'encaissement et d'exportation. Il illustre les différentes transitions d'états qui s'offrent à l'administrateur lors de la consultation du module d'Invoicing.

Le processus débute par un état d'accès à la liste globale des factures. Si aucune facture spécifique n'est sélectionnée, l'administrateur a la possibilité de déclencher une extraction des données financières sous forme de fichier \textbf{Excel/CSV} pour un traitement externe.

Lorsqu'une facture est sélectionnée, le système oriente le flux via un nœud de décision (\textit{Choice}) selon l'action choisie :
\begin{itemize}[label=\textcolor{royalBlue}{\rule{1.2ex}{1.2ex}}, leftmargin=*, itemsep=0.1cm]
    \item \textcolor{primaryBlue}{\textbf{Recouvrement :}} Transition vers l'état d'enregistrement des références de paiement, entraînant une mise à jour du statut vers \textit{PAYÉE} et la génération éventuelle d'un reçu via des actions internes.
    \item \textcolor{primaryBlue}{\textbf{Vérification des échéances :}} Le système évalue une condition de garde. Si la facture est toujours en attente et que la date actuelle dépasse la date d'échéance convenue, l'entité bascule automatiquement à l'état \textit{EN\_RETARD} et une relance par courrier électronique est déclenchée.
    \item \textcolor{primaryBlue}{\textbf{Consultation :}} Transition vers l'état de préparation et de téléchargement direct du document PDF archivé.
\end{itemize}

Indépendamment de la transition spécifique empruntée (modification ou consultation), le système clôture le cycle de vie en convergeant vers un état de consignation dans un journal d'audit, afin d'assurer une traçabilité complète des manipulations financières.

\vspace{0.4cm}

\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{images/etat-transition-recouvrement.png} 
    \vspace{-0.9cm}
    \caption{Diagramme d'état-transition : Gestion, relances et recouvrement des factures}
    \label{fig:etat_transition_recouvrement}
\end{figure}
...
\vspace{-0.4cm}
Dans ce chapitre, nous avons détaillé la phase de conception logicielle de notre projet en nous appuyant sur les standards du langage de modélisation UML. À travers l'élaboration progressive des diagrammes de cas d'utilisation, de classes, de séquence et d'état-transition, nous avons pu traduire les exigences métiers complexes des modules \textit{Invoicing} et \textit{Worktime} en spécifications techniques claires et rigoureuses.

Cette démarche de modélisation approfondie nous a permis de structurer une architecture robuste pour la plateforme KAFA'AT EMS. Nous avons ainsi pu anticiper le cycle de vie des entités et la logique des processus critiques, tels que la validation hiérarchique des pointages, le moteur de génération massive des factures, et la gestion des habilitations, tout en garantissant la traçabilité et l'intégrité des données financières.
...
La figure \ref{fig:architecture_globale} illustre cette architecture globale, mettant en évidence la communication fluide entre la couche de présentation (Angular), la logique métier centralisée (Spring Boot) et la persistance des données (PostgreSQL). \cite{kinsta_arch}
...
\begin{itemize}[label=\textcolor{royalBlue}{\rule{1.2ex}{1.2ex}}, leftmargin=*, itemsep=0.05cm]
    ...
    \item \textcolor{primaryBlue}{\textbf{Génération (JasperReports) :}} Dédiée à la création dynamique des factures au format PDF. \cite{jasperreports}
\end{itemize}
...
\begin{itemize}[label=\textcolor{royalBlue}{\rule{1.2ex}{1.2ex}}, leftmargin=*, itemsep=0.05cm]
    \item \textcolor{primaryBlue}{\textbf{Composants \& Templates (PrimeNG / SCSS) :}} Blocs de construction de l'interface. Chaque composant encapsule sa logique métier, sa structure HTML et son style, en intégrant les éléments riches de PrimeNG. \cite{primeng}
...
Cette approche moderne a considérablement optimisé notre flux de travail, résolvant les problèmes classiques de compatibilité inter-environnements et apportant une robustesse de niveau entreprise à l'ensemble du système. \cite{docker_docs}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.35\textwidth]{images/confluence-logo.png}
    \vspace{-0.4cm}
...
En parfaite complémentarité avec Jira, Confluence a été exploité comme espace de travail centralisé pour la gestion documentaire de notre projet au sein de l'environnement NTT DATA. 

Cet outil collaboratif nous a permis de capitaliser les connaissances, de rédiger les spécifications techniques et fonctionnelles des modules \textit{Invoicing} et \textit{Worktime}, et de maintenir une traçabilité de nos décisions architecturales. Son intégration native avec Jira a garanti une synchronisation fluide entre notre documentation et le suivi Agile de nos \textit{Sprints}.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.22\textwidth]{images/gitlab-logo.png}
    \vspace{-0.4cm}
...
\subsection{Interface d'authentification et sécurité}

\vspace{-0.4cm}
Pour garantir la sécurité et la confidentialité des données de la plateforme KAFA'AT EMS, l'accès est strictement contrôlé à travers un système d'authentification complet et sécurisé, divisé en plusieurs étapes clés.

\vspace{-0.5cm}

\subsubsection{Connexion principale}

\vspace{-0.4cm}

L'utilisateur doit initialement s'authentifier via son adresse e-mail professionnelle (domaine \textit{nttdata.com}) et son mot de passe.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.94\textwidth]{images/login-page.png}
    \vspace{-0.4cm}
    \caption{Interface d'authentification principale}
    \label{fig:login_main}
\end{figure}

\vspace{-0.3cm}
Dans le cas où les informations saisies sont erronées, le système bloque immédiatement l'accès. Une notification visuelle claire (sous forme de \textit{Toast} rouge) informe l'utilisateur que les identifiants sont incorrects, tout en appliquant les bonnes pratiques de sécurité pour éviter l'énumération de comptes.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.94\textwidth]{images/login-error.png}
    \vspace{-0.4cm}
    \caption{Gestion des erreurs de saisie lors de l'authentification}
    \label{fig:login_error}
\end{figure}

\vspace{-1.0cm}

\subsubsection{Authentification à double facteur (2FA)}

\vspace{-0.4cm}

Afin de renforcer le niveau de sécurité des accès au système, nous avons intégré un mécanisme d'authentification à double facteur (2FA). Après la validation réussie du mot de passe, le système exige la saisie d'un code dynamique à 6 chiffres généré par une application d'authentification tierce, telle que \textit{Microsoft Authenticator}.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.94\textwidth]{images/login-2fa.png}
    \vspace{-0.4cm}
    \caption{Interface de vérification à double facteur (MFA/2FA)}
    \label{fig:login_2fa}
\end{figure}

\vspace{-1.0cm}

\subsubsection{Récupération du compte}

\vspace{-0.4cm}

Pour offrir une autonomie aux utilisateurs en cas d'oubli de leurs identifiants, une interface de récupération a été mise en place. En saisissant son adresse e-mail professionnelle, l'utilisateur déclenche l'envoi d'un lien sécurisé et temporaire vers sa boîte de réception, lui permettant de réinitialiser son mot de passe en toute sécurité.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.94\textwidth]{images/forgot-password.png}
    \vspace{-0.4cm}
    \caption{Interface de récupération de mot de passe}
    \label{fig:forgot_pwd}
\end{figure}

\vspace{-0.4cm}

\subsection{Vue d'ensemble de la plateforme}

\vspace{-0.4cm}
Après une authentification réussie, l'utilisateur accède à l'espace de travail principal de la plateforme KAFA'AT EMS.

\subsubsection{Tableau de bord (Dashboard)}

\vspace{-0.4cm}

Ce tableau de bord centralise les indicateurs clés du \textit{Staffing}. Il synthétise les métriques essentielles (effectif global, recrutements, départs et \textit{Turnover}) et intègre un graphique d'évolution annuelle du \textit{Headcount} pour appuyer rapidement les décisions stratégiques.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.94\textwidth]{images/dashboard-staffing.png}
    \vspace{-0.4cm}
    \caption{Tableau de bord de gestion des effectifs et indicateurs RH}
    \label{fig:dashboard_staffing}
\end{figure}

\vspace{-1.0cm}

\subsubsection{Annuaire des collaborateurs (Vue en cartes)}

\vspace{-0.4cm}
Cette interface ergonomique sous forme de cartes (\textit{Card View}) résume les informations clés de chaque collaborateur (identifiant, département, expérience et statut). Elle intègre également des outils de recherche, de filtrage et d'exportation pour une gestion fluide et rapide des profils.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.94\textwidth]{images/employees-card-view.png}
    \vspace{-0.4cm}
    \caption{Interface de gestion des collaborateurs en vue "Cartes"}
    \label{fig:employees_cards}
\end{figure}


\subsection{Module de Facturation (Invoicing)}

\vspace{-0.4cm}
Ce module constitue le cœur de notre contribution technique. La première étape du processus de facturation nécessite une structuration robuste des entités financières de l'entreprise, à savoir les « Services » et leurs « Équipes » (\textit{Service Teams}).
...
\vspace{-0.4cm}
L'interface de gestion des services permet de configurer les unités d'affaires principales. Pour une ergonomie optimale, elle propose deux modes d'affichage : une vue en cartes pour un aperçu rapide, et une vue en liste détaillée avec des lignes extensibles exposant les projets rattachés, les responsables et le statut du service.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.94\textwidth]{images/service-cards.png}
    \vspace{-0.5cm}
    \caption{Interface de gestion des services (Vue en cartes)}
    \label{fig:service_cards}
\end{figure}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.94\textwidth]{images/service-list.png}
    \vspace{-0.5cm}
    \caption{Vue en liste avec détails extensibles des services}
    \label{fig:service_list}
\end{figure}

\vspace{-0.2cm}
La création d'un nouveau service s'effectue via une fenêtre modale structurée en plusieurs étapes (\textit{Stepper}). Cette approche guide l'utilisateur, segmente les informations requises (détails du service, projets, type) et minimise les erreurs de saisie.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.94\textwidth]{images/service-create.png}
    \vspace{-0.5cm}
    \caption{Formulaire de création d'un service (Étape 1 du Stepper)}
    \label{fig:service_create}
\end{figure}

\vspace{-1.1cm}

\subsubsection{Gestion des Équipes de Service (Service Teams)}

\vspace{-0.4cm}
Chaque service global est subdivisé en équipes opérationnelles (\textit{Service Teams}). Cette interface fait le pont entre la facturation et l'opérationnel en intégrant des paramètres techniques tels que les clés Jira, la localisation des équipes et le modèle de collaboration. Tout comme pour les services, l'affichage bascule dynamiquement entre une vue synthétique (cartes) et une vue analytique (liste).

\begin{figure}[H]
    \centering
    \includegraphics[width=0.94\textwidth]{images/service-teams-cards.png}
    \vspace{-0.5cm}
    \caption{Interface des équipes de service (Vue en cartes)}
    \label{fig:service_teams_cards}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.94\textwidth]{images/service-teams-list.png}
    \vspace{-0.4cm}
    \caption{Vue en liste des équipes avec paramètres techniques détaillés}
    \label{fig:service_teams_list}
\end{figure}

\vspace{-0.4cm}
La modification et le paramétrage d'une équipe de service sont gérés par un formulaire modal avancé. Cette interface permet d'ajuster finement les attributs de l'équipe (langue, clés Jira, modèle de gestion, extensions) tout en affichant des règles de validation strictes en temps réel, garantissant ainsi l'intégrité et l'exhaustivité des données avant toute soumission.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.94\textwidth]{images/service-team-edit.png}
    \vspace{-0.4cm}
    \caption{Interface de paramétrage d'une équipe de service avec validation dynamique}
    \label{fig:service_team_edit}
\end{figure}

 \vspace{-1.1cm}

\subsubsection{Gestion des Groupes de Facturation}

\vspace{-0.4cm}
Les « Groupes » représentent l'entité organisationnelle de plus haut niveau dans le module Invoicing. Ils permettent d'agréger les clients, les services et les équipes pour structurer les processus financiers. L'interface principale offre une vue d'ensemble sous forme de cartes, facilitant l'accès rapide aux métriques de chaque groupe (nombre de clients, services rattachés et combinaisons actives).

\begin{figure}[H]
    \centering
    \includegraphics[width=0.94\textwidth]{images/finance-groups-cards.png}
    \vspace{-0.4cm}
    \caption{Interface de gestion des groupes de facturation (Vue en cartes)}
    \label{fig:groups_cards}
\end{figure}

\vspace{-0.2cm}
Pour assurer une facturation précise et automatisée, le système repose sur des « Combinaisons de saisie » (\textit{Data entry combinations}). Cette interface critique permet de lier de manière cohérente un client, un service, une équipe opérationnelle et un compte financier (ex: code projet Jira). C'est cette matrice stricte qui conditionnera la validation des pointages (\textit{Worktime}) et la génération future des factures.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.94\textwidth]{images/finance-groups-combinations.png}
    \vspace{-0.4cm}
    \caption{Matrice de configuration des combinaisons de facturation}
    \label{fig:groups_combinations}
\end{figure}

\vspace{1.2cm}
Enfin, la sécurité et la traçabilité des données financières sont garanties par un module de gestion des permissions granulaire. Cette fenêtre modale permet d'attribuer des rôles spécifiques (Contrôle total, Lecture/Écriture, Lecture seule) aux administrateurs et aux membres pour chaque groupe de manière isolée.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.94\textwidth]{images/finance-groups-permissions.png}
    \vspace{-0.5cm}
    \caption{Interface de gestion des permissions et des rôles d'accès par groupe}
    \label{fig:groups_permissions}
\end{figure}

\vspace{-1.1cm}

\subsubsection{Suivi des Pointages (Work time)}

\vspace{-0.4cm}
Au sein d'un groupe spécifique, l'onglet \textit{Work time} présente une matrice mensuelle détaillée des pointages. Il croise les collaborateurs avec leurs combinaisons de saisie et met en évidence les exceptions (congés, maladies, jours fériés) via des codes couleurs stricts, assurant ainsi une base fiable pour la facturation.

\vspace{-0.1cm}
\begin{figure}[H]
    \centering
    \includegraphics[width=0.94\textwidth]{images/finance-worktime.png}
    \vspace{-0.5cm}
    \caption{Matrice de suivi mensuel des pointages et des exceptions}
    \label{fig:finance_worktime}
\end{figure}

\subsubsection{Régularisations et Ajustements (Adjustments)}

\vspace{-0.4cm}
La réalité opérationnelle nécessite souvent des corrections. L'onglet \textit{Adjustments} permet de gérer ces écarts (heures supplémentaires, pénalités, heures de service non facturables). L'interface liste toutes les régularisations avec leurs impacts financiers directs (taux, montants facturés au client vs coûts internes).

\begin{figure}[H]
    \centering
    \includegraphics[width=0.94\textwidth]{images/finance-adjustments-list.png}
    \vspace{-0.5cm}
    \caption{Liste des régularisations et ajustements financiers appliqués}
    \label{fig:finance_adjustments_list}
\end{figure}

\vspace{-0.4cm}
L'ajout d'un nouvel ajustement se fait via un formulaire dynamique. Selon le type d'ajustement sélectionné (ex: \textit{Extra Hours}), le formulaire adapte ses champs pour exiger les informations pertinentes (taux spécifique, justification, nombre d'heures exact), garantissant la traçabilité de chaque modification.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.94\textwidth]{images/finance-adjustment-add.png}
    \vspace{-0.5cm}
    \caption{Formulaire dynamique de saisie d'un ajustement}
    \label{fig:finance_adjustment_add}
\end{figure}

\subsubsection{Consolidation et Facturation (Billing)}

\vspace{-0.4cm}
L'onglet \textit{Billing} représente l'aboutissement du processus. Il s'agit d'une vue consolidée qui agrège automatiquement les données de pointage (\textit{Worktime}) et les corrections (\textit{Adjustments}). Le système calcule les montants finaux en appliquant les tarifs négociés (taux interne \textit{Center Rate} et taux de facturation \textit{Client Rate}), générant ainsi les lignes prêtes à être facturées, avec des sous-totaux précis par combinaison.

\vspace{-0.2cm}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.93\textwidth]{images/finance-billing.png}
    \vspace{-0.5cm}
    \caption{Vue consolidée de facturation agrégeant pointages et ajustements}
    \label{fig:finance_billing}
\end{figure}

\vspace{-1.1cm}

\subsubsection{Génération de Rapports et Exportation (Excel)}

\vspace{-0.4cm}
Pour faciliter le reporting financier, une fonctionnalité d'exportation vers Excel a été intégrée. L'utilisateur peut paramétrer finement son fichier via une fenêtre modale en sélectionnant les modules souhaités (\textit{Work time}, \textit{Adjustment}, \textit{Billing}), la période et l'équipe de service.

\vspace{-0.2cm}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.93\textwidth]{images/export-modal.png}
    \vspace{-0.5cm}
    \caption{Interface de paramétrage dynamique de l'exportation des données}
    \label{fig:export_modal}
\end{figure}

\vspace{-0.2cm}
Le système génère dynamiquement un classeur Excel structuré en plusieurs feuilles. Afin d'offrir une expérience optimale et un fichier directement exploitable, l'export conserve fidèlement la logique métier et la charte visuelle de l'application web. Les codes couleurs des exceptions sont maintenus, les cellules sont pré-formatées (devises, nombres) et les données sont organisées de manière claire.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.94\textwidth]{images/export-excel-worktime.png}
    \vspace{-0.5cm}
    \caption{Aperçu du fichier généré : Feuille formatée des pointages (Work time)}
    \label{fig:export_excel_worktime}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.94\textwidth]{images/export-excel-adjustments.png}
    \vspace{-0.5cm}
    \caption{Aperçu du fichier généré : Feuille détaillée des ajustements (Adjustments)}
    \label{fig:export_excel_adjustments}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.94\textwidth]{images/export-excel-billing.png}
    \vspace{-0.5cm}
    \caption{Aperçu du fichier généré : Feuille consolidée de facturation (Billing)}
    \label{fig:export_excel_billing}
\end{figure}

\vspace{-1.0cm}

\subsubsection{Processus de Validation et Clôture (Process)}

\vspace{-0.4cm}
Pour sécuriser le cycle financier, un workflow de validation strict a été mis en place. Le tableau de bord du processus offre une vue globale sur l'état d'avancement de chaque service, structuré en plusieurs étapes d'approbation (de l'estimation initiale à la clôture définitive).

\begin{figure}[H]
    \centering
    \includegraphics[width=0.94\textwidth]{images/process-dashboard.png}
    \vspace{-0.4cm}
    \caption{Tableau de bord de suivi du processus de facturation par service}
    \label{fig:process_dashboard}
\end{figure}

\vspace{3.2cm}
Une vue détaillée permet d'auditer les lignes de facturation spécifiques à un service pour un mois donné. Elle intègre une frise chronologique interactive indiquant le statut de confirmation (confirmé ou non confirmé) des différents mois de l'année.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.94\textwidth]{images/process-details.png}
    \vspace{-0.4cm}
    \caption{Vue détaillée des montants facturés et suivi chronologique des statuts}
    \label{fig:process_details}
\end{figure}

\vspace{-0.2cm}
En cas de modification rétroactive des pointages ou des tarifs, le système dispose d'une fonctionnalité de régénération. Cette option offre la flexibilité de recalculer automatiquement les montants sur un ou plusieurs mois, à condition que la période ciblée n'ait pas encore été définitivement clôturée.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.94\textwidth]{images/process-regenerate.png}
    \vspace{-0.4cm}
    \caption{Interface modale de régénération des données financières sur plusieurs mois}
    \label{fig:process_regenerate}
\end{figure} 

\vspace{3.2cm}
L'interface permet aux responsables financiers de valider manuellement les données mensuelles. Un menu contextuel, accessible directement depuis la frise chronologique, offre la possibilité de verrouiller le mois en cours (Action de confirmation) ou de basculer vers la vue de révision.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.94\textwidth]{images/process-confirm-action.png}
    \vspace{-0.4cm}
    \caption{Action de confirmation mensuelle via la frise chronologique interactive}
    \label{fig:process_confirm_action}
\end{figure}

\vspace{-0.2cm}
Une fois la période validée, le statut bascule officiellement sur « Confirmé ». Pour des raisons de conformité et de sécurité, le système génère immédiatement une trace d'audit (\textit{Audit Trail}). L'interface affiche clairement l'identité de l'utilisateur ayant approuvé les données ainsi que l'horodatage exact de l'action, garantissant ainsi une transparence totale du processus d'invoicing.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.94\textwidth]{images/process-confirmed-status.png}
    \vspace{-0.4cm}
    \caption{État confirmé avec traçabilité d'audit (Utilisateur et horodatage)}
    \label{fig:process_confirmed_status}
\end{figure}

\section*{Conclusion}
\addcontentsline{toc}{section}{Conclusion} % Bach t-ban f l-fahras ila bghiti

\vspace{-0.4cm}
En conclusion, ce chapitre a permis d'illustrer la concrétisation technique de notre travail. Nous y avons détaillé les étapes cruciales de la réalisation du module \textit{Invoicing} pour la plateforme KAFA'AT EMS, de la configuration de l'environnement de développement à l'implémentation complète des interfaces utilisateur. L'adoption d'une architecture full-stack robuste, couplée à l'utilisation d'Angular et de PrimeNG, a rendu possible la conception d'interfaces ergonomiques et fluides, capables de gérer la complexité des processus de facturation et de suivi des pointages (\textit{Worktime}). 

De plus, l'intégration de mécanismes de sécurité avancés (tels que l'authentification à double facteur) et de fonctionnalités à forte valeur ajoutée (comme l'exportation dynamique vers Excel et la traçabilité des validations) garantit un produit final fiable et aligné avec les standards professionnels rigoureux de l'entreprise. Ce chapitre démontre ainsi la traduction réussie des exigences conceptuelles en une solution logicielle performante, sécurisée et parfaitement adaptée aux besoins des utilisateurs finaux.
...
\noindent
\textcolor{royalBlue}{\rule{0.25\textwidth}{1pt}}
\hfill
{\fontsize{24}{28}\selectfont\color{royalBlue}\bfseries Conclusion et Perspectives}
...
Ce projet de fin d'études chez NTT DATA a permis la conception et l'implémentation full-stack du module \textit{Invoicing} pour la plateforme KAFA'AT EMS. En adoptant la méthodologie Agile Scrum, nous avons bâti une architecture robuste (Angular, Spring Boot) répondant aux exigences complexes de gestion financière, de suivi des pointages et de facturation automatisée.

Cette expérience fut doublement enrichissante : techniquement, par la maîtrise d'un cycle de développement complet, sécurisé (MFA, JWT) et ergonomique ; et professionnellement, par une intégration réussie au sein d'une équipe dynamique d'une multinationale. Le module livré est aujourd'hui fonctionnel et prêt à optimiser le quotidien des équipes financières.
...
Bien que le système réponde parfaitement au cahier des charges, plusieurs axes d'amélioration peuvent le rendre encore plus intelligent et proactif :
...
\begin{itemize}
   ...
   \vspace{-0.5cm}
    \item \textcolor{primaryBlue}{\textbf{Interopérabilité ERP (SAP / Oracle) :}} Synchronisation automatisée des factures avec les systèmes comptables externes via API et Webhooks.
    
    \vspace{-0.5cm}
    \item \textcolor{primaryBlue}{\textbf{Analytique prédictive :}} Création de tableaux de bord anticipant les revenus et l'évolution du \textit{Turnover}.
    
    \vspace{-0.5cm}
    \item \textcolor{primaryBlue}{\textbf{Intelligence Artificielle (ML) :}} Intégration de modèles de Machine Learning pour la détection automatique d'anomalies financières avant la clôture.
    
    \vspace{-0.5cm}
    \item \textcolor{primaryBlue}{\textbf{Assistant Virtuel (RAG) :}} Mise en place d'un système conversationnel permettant d'interroger instantanément l'historique de facturation en langage naturel.
\end{itemize}

\vspace{-0.4cm}
Ces évolutions confirmeront la transition de ce module vers une solution financière entièrement intelligente et automatisée.
...
\noindent
\textcolor{royalBlue}{\rule{0.60\textwidth}{1pt}}
\hfill
{\fontsize{24}{28}\selectfont\color{royalBlue}\bfseries Références}\par
...
\vspace{3cm} % Kat-tle3 l-liste l-fou9 3nd l-khat

\begin{thebibliography}{99}
    \raggedright % <-- L-fix li kay-7yed l-underfull hbox
    \setlength{\itemsep}{0.2cm} % L'espace bin les références

    \bibitem{ntt_global} 
    \textbf{NTT DATA Global.} \textit{About Us, Industries \& Services}. \newline
    Disponible sur : \url{https://www.nttdata.com/global/en/}

    \bibitem{ntt_maroc} 
    \textbf{NTT DATA Maroc.} \textit{À propos de nous et Alliances}. \newline
    Disponible sur : \url{https://mar.nttdata.com/}

    \bibitem{scrum_guide} 
    \textbf{Atlassian Agile Coach.} \textit{Qu'est-ce que la méthodologie Scrum ?}. \newline
    Disponible sur : \url{https://www.atlassian.com/fr/agile/scrum}

    \bibitem{jira_software} 
    \textbf{Atlassian.} \textit{Jira Software : Outil de gestion de projet Agile}. \newline
    Disponible sur : \url{https://www.atlassian.com/fr/software/jira}

    \bibitem{gantt} 
    \textbf{Gantt.com.} \textit{Qu'est-ce qu'un diagramme de Gantt ?}. \newline
    Disponible sur : \url{https://www.gantt.com/fr/}

    \bibitem{uml_ibm} 
    \textbf{IBM Documentation.} \textit{Diagrammes et modèles UML}. \newline
    Disponible sur : \url{https://www.ibm.com/docs/fr/rational-soft-arch/9.5?topic=diagrams-uml-models}

    \bibitem{spring_boot} 
    \textbf{Spring.} \textit{Spring Boot Reference Documentation}. \newline
    Disponible sur : \url{https://docs.spring.io/spring-boot/docs/current/reference/html/}

    \bibitem{angular_docs} 
    \textbf{Google.} \textit{Angular : The modern web developer's platform}. \newline
    Disponible sur : \url{https://angular.io/docs}

    \bibitem{kinsta_arch} 
    \textbf{Kinsta.} \textit{Architecture d'application web}. \newline
    Disponible sur : \url{https://kinsta.com/fr/blog/architecture-application-web/}

    \bibitem{postgresql} 
    \textbf{PostgreSQL Global Development Group.} \textit{PostgreSQL: The World's Most Advanced Open Source Relational Database}. \newline
    Disponible sur : \url{https://www.postgresql.org/docs/}

    \bibitem{primeng} 
    \textbf{PrimeTek.} \textit{PrimeNG : The Most Complete Angular UI Component Library}. \newline
    Disponible sur : \url{https://primeng.org/}

    \bibitem{jwt_auth0} 
    \textbf{Auth0.} \textit{JSON Web Token (JWT) Introduction}. \newline
    Disponible sur : \url{https://jwt.io/introduction}

    \bibitem{jasperreports} 
    \textbf{Jaspersoft Community.} \textit{JasperReports Library : Open Source Reporting}. \newline
    Disponible sur : \url{https://community.jaspersoft.com/project/jasperreports-library}

    \bibitem{docker_docs} 
    \textbf{Docker Inc.} \textit{Docker Documentation}. \newline
    Disponible sur : \url{https://docs.docker.com/}

    \bibitem{git_scm} 
    \textbf{Git.} \textit{Git - Fast Version Control System}. \newline
    Disponible sur : \url{https://git-scm.com/doc}

    \bibitem{gitlab_docs} 
    \textbf{GitLab.} \textit{GitLab Documentation}. \newline
    Disponible sur : \url{https://docs.gitlab.com/}
